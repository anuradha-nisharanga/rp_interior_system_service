window.addEventListener('load',()=>{
    ...
    userPrivilege = ajaxGetRequest("/privilege/by-logged-user-module/sale");
...
const refreshGrnTable = () =>{
    ...
    fillDataIntoTable(orderTable, GRNList ,displayProperty ,refillPGrnForm, deleteGRN, printPOrder, true, userPrivilege);
    ...
    GRNList.forEach((element, index) => {
        if(element.grnStatus.name === "Deleted"){
            if (userPrivilege.delete) {
                orderTable.children[1].children[index].children[7].children[1].disabled = true; //you can also use disabled
            }
        }
    });
